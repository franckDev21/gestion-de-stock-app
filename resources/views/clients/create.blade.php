      ...
      <form method="POST" action="{{ route('clients.store') }}">
          ...
          <div class="ml-1">
            ...
            <x-input id="tel" placeholder="Entrer numéro de téléphone du client"
                class="block w-full  mt-1" type="tel" name="tel" :value="old('tel')"  />
            @error('tel')
                <span class="text-sm text-red-400 block">{{ $message }}</span>
            @enderror
          </div>
          ...
          <div class="ml-1">
            ...
            <x-input id="address" placeholder="Adresse" class="block w-full mt-1" type="text" name="address" :value="old('address')"  />
            @error('address')
                <span class="text-sm text-red-400 block">{{ $message }}</span>
            @enderror
          </div>
          ...
          <div class="flex items-center justify-end mt-5">
              ...
              <x-button class="text-center flex justify-center items-center">
                  {{ __('Enregistrer le nouveau client ') }}
              </x-button>
          </div>
      </form>
  </div>
