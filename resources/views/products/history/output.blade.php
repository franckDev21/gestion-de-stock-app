                        ...
                        @foreach ($historiques as $history)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="py-3 px-4 text-left flex flex-col">
                                    <span><span>{{ $history->quantite }} </span>
                                        {{ $history->is_unite ? 'Unitée(s)' : $history->product->type_approvionement . '(s)' }}
                                    </span>
                                ...
                                <td class="py-1 px-4 text-left">
                                    ...
                                    <span class=" py-1 rounded-full font-semibold">{{ $history->user->firstname }}
                                        {{ $history->user->lastname }}</span>
                                </td>

                                <td
                                    class="py-3 px-4 text-left {{ $history->type === 'SORTIE' ? 'text-red-500' : 'text-green-500' }}">
                                    {{ $history->type }}
                                </td>
                                ...
                                <td class="py-3 px-4 text-left">
                                    <div class="flex item-center justify-center">
                                        <a href="{{ route('history.index.product', $history->product->id) }}"
                                            title="voir tous les entrées et sorties du {{ $history->product->nom }}"
